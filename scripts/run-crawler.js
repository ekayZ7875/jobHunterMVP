...
async function main() {
  ...
  try {
    const startUrl = process.env.CRAWL_START_URL || 'https://weworkremotely.com/remote-jobs/search?term=node';
    const maxPages = parseInt(process.env.CRAWL_MAX_PAGES || '2', 10);

    console.log(`Calling crawlWeWorkRemotely({ startUrl: "${startUrl}", maxPages: ${maxPages} })`);
